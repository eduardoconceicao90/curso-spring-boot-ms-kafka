...
public class GeradorNotaFiscalService {
    ...
    @Value("classpath:reports/nota-fiscal.jasper")
    ...
    public byte[] gerarReportNotaFiscal(Pedido pedido){
        try (InputStream inputStream = notaFiscal.getInputStream()) {
            ...
            // Carrega o arquivo .jasper já compilado (mais rápido em produção)
            JasperReport jasperReport = (JasperReport) JRLoader.loadObject(inputStream);

            // Caso queira compilar o arquivo .jrxml em tempo de execução (útil em desenvolvimento)
